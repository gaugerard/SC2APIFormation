...
from SC2_FORMATION.PartieAvancee import DefendAgainstZergRush

import random


class BotZerg(sc2.BotAI):
    ...
    async def attack(self):
        ...
        if self.units(ZERGLING).idle.amount > 0:
            ...
            for zergling in self.units(ZERGLING).idle:
                ...
                if len(self.known_enemy_units) > 0:
                    ...
                    await self.do(zergling.attack(enemy_unit))

                else:
                    if self.units(ZERGLING).idle.amount > 40:

                        enemy_location = self.enemy_start_locations[0]
                        await self.do(zergling.attack(enemy_location))
...
if __name__ == "__main__":
    ...
    run_game(
        ...
        [
            Bot(Race.Zerg, BotZerg()),
            Bot(Race.Terran, DefendAgainstZergRush.BotTerran())
        ],
        realtime=True)  # time in second : 1800sec = 30 min
